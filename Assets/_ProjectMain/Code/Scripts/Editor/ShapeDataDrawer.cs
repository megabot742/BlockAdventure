...
public class ShapeDataDrawer : Editor
{
    ...
    private void DrawColumnsInputFields()
    {
        ...
        if ((ShapeDataInstance.columns != columsTemp || ShapeDataInstance.rows != rowsTemp) && ShapeDataInstance.columns > 0 && ShapeDataInstance.rows > 0)
        {
    ...
    private void DrawBoardTable()
    {
        ...
        for (var row = 0; row < ShapeDataInstance.rows; row++)
        {
            ...
            for (var column = 0; column < ShapeDataInstance.columns; column++)
            {
                ...
                if (ShapeDataInstance.board[row].column == null)
                {
                    Debug.LogError($"Column array at row {row} is null!");
                    return;
                }
